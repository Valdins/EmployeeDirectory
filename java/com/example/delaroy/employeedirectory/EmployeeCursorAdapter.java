...
public class EmployeeCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        // If the employee firstname is empty string or null, then use some default text
        // that says "Unknown", so the TextView isn't blank.
        if (TextUtils.isEmpty(employeeFirstName)) {
            employeeFirstName = context.getString(R.string.unknown);
        }

        if (TextUtils.isEmpty(employeeTitle)) {
            employeeTitle = context.getString(R.string.unknown);
        }

        // Update the TextViews with the attributes for the current employee
